...
import { UPDATE_ZONE } from "@utils/gql/zone/constant";
...
export default function EditZoneCard({ close, title, zone }) {
  ...
  const [mutationEditData, editData] = useMutation(UPDATE_ZONE);
            ...
            {({ errors, touched }) => (
              <Form className={cssModules.form}>
                ...
                <Box sx={{ height: "20vh", width: "100%" }}>
                  <MapContainer
                    className="Map"
                    center={{ lat: zone.lat, lng: zone.long }}
                    zoom={5}

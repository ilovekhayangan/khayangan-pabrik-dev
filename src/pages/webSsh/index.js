...
import SshCard from "@components/molecules/WebSshCard";
import LoadingBar from "@components/atoms/LoadingBar";
import { GET_SSH } from "@utils/gql/ssh/constant";
import ActionButton from "@components/atoms/ActionWebSsh";
import { GET_WEBSSH } from "@utils/gql/webSsh/constant";

const WebSSH = () => {
  const title = "Web SSH Menu";
  ...
  ] = useLazyQuery(GET_WEBSSH, {
    onCompleted: ({ wsshesConnection }) => {
      if (wsshesConnection?.data.length > 0) {
        setDataSource(
          wsshesConnection?.data.map((ssh) => ({
            ...ssh,
            key: ssh.id,
          }))
        );
    ...
    <div>
      ...
      <Box sx={{ px: "1.5rem", boxSizing: "border-box" }}>
        ...
        <div style={{ paddingTop: "0.25rem" }} id="top">
          <SingleBreadcrumbs title="Web SSH" route="setting/sshServer" />
        </div>
        ...
        >
          ...
          <div style={{ marginRight: 20 }}>
            ...
            >
              ...
              <Typography>Add Web SSH</Typography>
            </Button>
          </div>
        </Box>
        {dataLoading ? (

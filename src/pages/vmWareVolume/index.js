...
import { VM_VOLUME } from "@utils/gql/volume/constant";
import {
  REGION_FILTER,
  ZONE_FILTER,
  ZONE_BEFORE_FILTER,
...
export default function VmwareInstance() {
  ...
  const [getData, { loading: dataLoading, data: getVmData }] = useLazyQuery(
    VM_VOLUME,
    {
      onCompleted: ({ volumesConnection }) => {
        if (volumesConnection.data) {
          setDataSource(
            volumesConnection?.data.map((volume) => ({
              ...volume,
              key: volume.id,
            }))
          );
  ...
  const columns = [
    ...
    },
    {
      ...
      title: "Client",
      columnsRender: (row) => {
        return row?.updatedBy ? row.updatedBy?.firstName : "Admin MSP";
      },
    },
    {
      key: "size",
    ...
    },
    {
      ...
      title: "Attach To",
      columnsRender: (row) => {
        let volumeName = row.name.split("/")[1].replace(".vmdk", "");
        return `on Instance ${volumeName}`;
      },
    },
  ];
